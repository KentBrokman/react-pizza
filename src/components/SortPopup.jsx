    ...
    return (
        <div className="sort">
            <div className="sort__label">
                <svg
                    className={popupIsActive ? 'rotated' : ''}
                    width="10"
                    height="6"
                    viewBox="0 0 10 6"
                    fill="none"
                ...
                <b>Сортировка по:</b>
                <span onClick={showPopup}>{text[popupElementIsActive]}</span>
            </div>
            {popupIsActive && <div ref={popupRef}
                                   className="sort__popup">
                <ul>
                    {text.map((item, index) => {
                        return (
                            <li className={index === popupElementIsActive ? 'active' : ''}
